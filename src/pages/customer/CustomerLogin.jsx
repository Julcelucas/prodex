...
const CustomerLogin = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-blue-50 p-4">
      <Helmet>
        <title>{pt.auth.customerLoginTitle}</title>
      ...
      <Card className="w-full max-w-md shadow-xl border-blue-100">
        <CardHeader className="space-y-1">
          ...
          <CardTitle className="text-2xl font-bold text-center text-gray-900">
            {pt.auth.customerLoginHeader}
          </CardTitle>
          <CardDescription className="text-center text-gray-600">
            {pt.auth.enterCredentials}
          </CardDescription>
        </CardHeader>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button
              ...
              disabled={loading}
            >
              {loading ? pt.auth.loggingIn : pt.auth.loginBtn}
            </Button>
          </form>
        </CardContent>
        ...
        <CardFooter className="flex flex-col space-y-3">
          <p className="text-sm text-gray-600">
            {pt.auth.dontHaveAccount}{' '}
            <Link to="/customer/register" className="text-blue-600 hover:text-blue-700 font-semibold">
              {pt.auth.registerHere}
            </Link>
          </p>
          <Link
            to="/"
